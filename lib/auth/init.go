...
func generateSelfSignedCA(cfg *InitConfig, hsmClient hsm.Client, caType types.CertAuthType) (*types.CertAuthorityV2, error) {
	sshPrivateKey, err := hsmClient.GenerateRSA()
	if err != nil {
		return nil, trace.Wrap(err)
	}
	keyType := hsm.KeyType(sshPrivateKey)
	sshCryptoSigner, err := hsmClient.GetSigner(sshPrivateKey)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	...
	tlsPrivateKey, err := hsmClient.GenerateRSA()
	if err != nil {
		return nil, trace.Wrap(err)
	}
	tlsSigner, err := hsmClient.GetSigner(tlsPrivateKey)
	if err != nil {
		return nil, trace.Wrap(err)
	}
