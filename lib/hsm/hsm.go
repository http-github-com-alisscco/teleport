...
type Client interface {
	...
	// The returned identifier should be passed to GetSigner to get a
	// crypto.Signer including the public key.
	GenerateRSA() (key []byte, err error)
...
func NewClient(config *ClientConfig) (Client, error) {
	...
	if config.Path != "" {
		...
		return newNullClient(config), nil
	}
}
...
// GenerateRSA creates a new RSA private key and returns its identifier.  The
// returned identifier should be passed to GetSigner to get a crypto.Signer
// including the public key.
func (c *pkcs11Client) GenerateRSA() ([]byte, error) {
	id, err := uuid.NewRandom()
	if err != nil {
		return nil, err
	}

	_, err = c.ctx.GenerateRSAKeyPair(id[:], teleport.RSAKeySize)
	if err != nil {
		return nil, err
	}
...
func (c *pkcs11Client) GetSigner(rawKey []byte) (crypto.Signer, error) {
	...
	case types.PrivateKeyType_PKCS11:
		...
		signer, err := c.ctx.FindKeyPair(pkcs11ID, nil)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		return signer, nil
...
func newNullClient(config *ClientConfig) Client {
	return &nullClient{
		rsaKeyPairSource: config.RSAKeyPairSource,
	}
}

// GenerateRSA creates and returns a new RSA private key. The returned key
// should be passed to GetSigner to get a crypto.Signer including the public
// key.
func (c *nullClient) GenerateRSA() ([]byte, error) {
	priv, _, err := c.rsaKeyPairSource("")
	return priv, trace.Wrap(err)
}

// GetSigner returns a crypto.Signer for the given key raw private key.
func (c *nullClient) GetSigner(rawKey []byte) (crypto.Signer, error) {
	signer, err := utils.ParsePrivateKeyPEM(rawKey)
	return signer, trace.Wrap(err)
}
...
func (c *nullClient) GetTLSCertAndSigner(ca types.CertAuthority) ([]byte, crypto.Signer, error) {
	keyPairs := ca.GetActiveKeys().TLS
	if len(keyPairs) == 0 {
...
func (c *nullClient) GetSSHSigner(ca types.CertAuthority) (ssh.Signer, error) {
	keyPairs := ca.GetActiveKeys().SSH
	if len(keyPairs) == 0 {
		return nil, trace.NotFound("no SSH key pairs found in CA for %q", ca.GetClusterName())
	}
...
func (c *nullClient) selectJWTSigner(ca types.CertAuthority) (crypto.Signer, error) {
	keyPairs := ca.GetActiveKeys().JWT
	if len(keyPairs) == 0 {
		return nil, trace.NotFound("no JWT keypairs found")
	}
...
// GetJWTSigner returns the active JWT key used to sign tokens.
func (c *nullClient) GetJWTSigner(ca types.CertAuthority, clock clockwork.Clock) (*jwt.Key, error) {
	signer, err := c.selectJWTSigner(ca)
	if err != nil {
		return nil, trace.Wrap(err)
	}
