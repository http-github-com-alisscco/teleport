...
type proxySuiteOptions struct {
	...
	secondClusterPorts *InstancePorts
...
func newProxySuite(t *testing.T, opts ...proxySuiteOptionsFunc) *ProxySuite {
	options := proxySuiteOptions{
		mainClusterPorts:   oneProxyPortSetup(),
		secondClusterPorts: oneProxyPortSetup(),
	}
	...
	lc := NewInstance(InstanceConfig{
		...
		log:         testlog.FailureOnly(t),
		Ports:       options.secondClusterPorts,
	})
	suite := &ProxySuite{
		main:   rc,
		second: lc,
	...
	if options.trustedCluster != nil {
		tryCreateTrustedCluster(t, suite.second.Process.GetAuthServer(), options.trustedCluster)
		waitForTunnelConnections(t, suite.main.Process.GetAuthServer(), suite.second.Secrets.SiteName, 1)
	}

	return suite
}

func (p *ProxySuite) addNodeToSecondCluster(t *testing.T, tunnelNodeHostname string) {
	const (
	...
	nodeConfig := func() *service.Config {
		...
		tconf.AuthServers = []utils.NetAddr{
			{
				AddrNetwork: "tcp",
				Addr:        net.JoinHostPort(Loopback, p.second.GetPortWeb()),
			},
		}
	...
	_, err := p.second.StartNode(nodeConfig())
	require.NoError(t, err)

	err = utils.RetryStaticFor(deadline, nextIterWaitTime, func() error {
		if len(checkGetClusters(t, p.main.Tunnel)) < 2 && len(checkGetClusters(t, p.second.Tunnel)) < 2 {
	...
	err = waitForNodeCount(context.Background(), p.main, p.second.Secrets.SiteName, 2)
	require.NoError(t, err)
}
...
func withSecondClusterRoles(roles ...types.Role) proxySuiteOptionsFunc {
	return func(options *proxySuiteOptions) {
		options.secondClusterRoles = roles
	}
}

func withMainAndSecondClusterRoles(roles ...types.Role) proxySuiteOptionsFunc {
	return func(options *proxySuiteOptions) {
		withMainClusterRoles(roles...)(options)
		withSecondClusterRoles(roles...)(options)

	}
}

func withSecondConfig(fn func(suite *ProxySuite) *service.Config, configModFunctions ...func(config *service.Config)) proxySuiteOptionsFunc {
	return func(options *proxySuiteOptions) {
...
func withMainAndSecondTrustedClusterReset() proxySuiteOptionsFunc {
	return func(options *proxySuiteOptions) {
...
func withSecondClusterPorts(ports *InstancePorts) proxySuiteOptionsFunc {
	return func(options *proxySuiteOptions) {
		options.secondClusterPorts = ports
	}
}
...
func mainStandardConfig(t *testing.T) func(suite *ProxySuite) *service.Config {
...
func mainSecondConfig(t *testing.T) func(suite *ProxySuite) *service.Config {
	return func(suite *ProxySuite) *service.Config {
		lc := suite.second
		config := service.MakeDefaultConfig()
		config.DataDir = t.TempDir()
		...
		config.Auth.Preference.SetSecondFactor("off")
		config.Proxy.Enabled = true
		//config.Proxy.TLSProxyAddr.Addr = net.JoinHostPort(lc.Hostname, lc.GetPortProxyTLS())
...
func withTrustedCluster() proxySuiteOptionsFunc {
	return func(options *proxySuiteOptions) {
		options.updateRoleMappingFunc = func(t *testing.T, suite *ProxySuite) {
			...
			secondRole := suite.second.Secrets.Users[mustGetCurrentUser(t).Username].Roles[0]
